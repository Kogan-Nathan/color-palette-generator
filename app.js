//global selections and veriables
...
saveBtn.addEventListener('click', openPalettes);
closeSave.addEventListener('click', closePalettes);
...
function colorizeSliders(color, hue, brightness, saturation){
    ...
    brightness.style.backgroundImage = `linear-gradient(to right,${scaleBrightness(1)},${scaleBrightness(0.5)}, ${scaleBrightness(0)})`
...
function openPalettes(event){
    const popup = saveContainer.children[0];
    saveContainer.classList.add("active");
    popup.classList.add("active");
};

function closePalettes(event){
    const popup = saveContainer.children[0];
...
function savePalette(event){
    ...
    paletteObj.colors.forEach( smallColor =>{
        const samllDiv = document.createElement("div");
        samllDiv.style.backgroundColor = smallColor;
        preview.appendChild(samllDiv);
    });
